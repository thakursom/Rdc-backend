...
class YoutubeDashboardService {
    ...
    static async calculateAndSaveYoutubeDashboard(userId, role) {
        try {
            ...
            await YoutubeDashboardSnapshot.updateOne(
                { user_id: snapshotUserId },
                {
                    $set: snapshotData,
                },
    ...
    static async calculateYoutubeOverview(match) {
        const [result] = await Youtube.aggregate([
            ...
            {
                $facet: {
                    totalRevenue: [{ $group: { _id: null, v: { $sum: { $toDouble: "$total_revenue" } } } }],
                    totalStream: [{ $group: { _id: null, v: { $sum: { $toDouble: "$total_play" } } } }],
                    topRelease: [
                        { $group: { _id: "$asset_title", v: { $sum: { $toDouble: "$total_revenue" } } } },
                        { $sort: { v: -1 } }, { $limit: 1 }
                    ],
                    topTrack: [
                        {
                            $group: {
                                _id: { t: "$asset_title", r: "$asset_title", p: "$retailer", i: "$isrc_code" },
...
module.exports = YoutubeDashboardService;